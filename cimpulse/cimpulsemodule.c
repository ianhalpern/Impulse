...
static PyObject * cimpulse_getSnapshot( PyObject *self, PyObject *args, PyObject *kwargs ) {
	PyObject *magnitude;
	int fft = 0;
	...
	if ( ! fft ) {
		...
		for ( i = 0; i < CHUNK / 2; i += 2 ) {
			PyObject *x;
			x = PyFloat_FromDouble( fabs( ( (double) snapshot[ i ] / ( pow( 2, 16 ) / 2 ) + (double) snapshot[ i + 1 ] / ( pow( 2, 16 ) / 2 ) ) / 2 ) );
			PyTuple_SetItem( magnitude, i / 2, x );
		}
	} else {
		...
		if ( out != NULL ) {
			int i;
			for ( i = 0; i < CHUNK / 4; i++ ) {
				double tmp = (double) sqrt( pow( out[ i ][ 0 ], 2 ) + pow( out[ i ][ 1 ], 2 ) ) / (double) fft_max[ i ];
				if ( tmp > 1.0 ) tmp = 1.0;
				PyObject *x;
				x = PyFloat_FromDouble( tmp );
...
static PyMethodDef CimpulseMethods[ ] = {
	...
	{ "getSnapshot",  (PyCFunction)cimpulse_getSnapshot, METH_VARARGS | METH_KEYWORDS, "Returns the current audio snapshot from Pulseaudio." },
